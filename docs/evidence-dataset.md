...
- **Credential theft:** **`PWDumpX.exe`** executed on `SRL-DMZFTP`; a **confirmed-executed `ntdsutil` IFM extraction of the full NTDS.dit** on `SRL-DC` (spsql, launched via WMI and WinRM); and an **attempted** DC **SAM-hive copy from a Volume Shadow Copy** issued from `SRL-FILE` — command provably executed, copy *success* not independently corroborated. The per-technique confirmed-vs-attempted calibration is the centerpiece of `accuracy-report.md`.
